...
VisionAgent = Agent(
    ...
    Your role:
    - Use the `start_session_phases` tool to start a new chat only.
    - Use the `chat_with_phases` function for EVERY user message
    ...
    For tool `start_session_phases` and `chat_with_phases` usage:
    ...
    **CRITICAL RULES**: 
    ...
    - If `chat_state` is False, continue the conversation using the `chat_with_phases` tool
    ...
    """,
    tools=[start_session_phases, chat_with_phases]
)
